...
// =============================
// BFT Wynnum Landing Page — 28 Day Kickstart (No Prices)
// Owners: Ben & Pren / Head Coach: Christian / Coaches: Tyneale & Josh
// Address: 66 Edith St Wynnum 4178 / Phone: 0413496289 / Email: wynnum@bodyfittraining.com
// Forms: POST to Wingman CRM (swap YOUR_WINGMAN_ENDPOINT below)
// =============================
...
export default function BFTWynnumLanding() {
  ...
  // ---- Wingman CRM submit (swap endpoint) ----
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        name,
        email,
        phone, // Wingman fields: name, number, email (mapping phone -> number happens in the Netlify function)
      };
      ...
      // Replace with your real Wingman endpoint URL
      const res = await fetch("https://YOUR_WINGMAN_ENDPOINT/lead", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload),
      });

      if (!res.ok) throw new Error(`Wingman responded ${res.status}`);
      setSubmitted(true);
    } catch (err) {
      console.error(err);
      setError("Sorry, something went wrong. Please try again or contact us directly.");
  ...
  const Testimonial = ({ quote, name, subtitle, stars = 5 }) => (
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white text-slate-900">
      ...
      <header className="sticky top-0 z-50 bg-white/80 backdrop-blur border-b border-slate-200">
        <div className="max-w-6xl mx-auto px-4">
          <div className="flex items-center justify-between h-16">
            <a href="#home" className="flex items-center gap-2 font-bold"><Dumbbell className="w-5 h-5" /><span>BFT Wynnum</span></a>
            <nav className="hidden md:flex items-center gap-6 text-sm">
              ...
              <a href="#results" className="hover:text-slate-900 text-slate-600">Results</a>
            ...
            <div className="hidden md:block"><Button asChild className="rounded-2xl"><a href="#offer">Start 28 Day Kickstart</a></Button></div>
          </div>
        </div>
      </header>
      ...
      <section id="home" className="relative">
        <div className="absolute inset-0 -z-10">
          <img src="https://images.unsplash.com/photo-1517836357463-d25dfeac3438?q=80&w=2000&auto=format&fit=crop" alt="Group training at a fitness studio" className="w-full h-full object-cover opacity-20" loading="eager" />
        </div>
        <div className="max-w-6xl mx-auto px-4 py-20 md:py-28 grid md:grid-cols-2 gap-10 items-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            ...
            <div className="mt-6 flex flex-col sm:flex-row gap-3">
              <Button size="lg" asChild className="rounded-2xl"><a href="#offer" className="flex items-center gap-2">Start 28 Day Kickstart <ArrowRight className="w-4 h-4" /></a></Button>
              <Button size="lg" variant="outline" asChild className="rounded-2xl"><a href="#timetable">View Timetable</a></Button>
            </div>
        ...
        <div className="max-w-6xl mx-auto px-4 grid md:grid-cols-3 gap-6 items-stretch">
          <div className="md:col-span-2">
            ...
            <div className="mt-6 flex gap-3">
              <Button asChild size="lg" className="rounded-2xl"><a href="#home">Get Started</a></Button>
              <Button asChild size="lg" variant="outline" className="rounded-2xl"><a href="#faqs">What’s included</a></Button>
            </div>
          </div>
          ...
          <div className="mt-8 grid sm:grid-cols-2 md:grid-cols-3 gap-6">
            {COACHES.map((c) => (
              <Card key={c.name} className="overflow-hidden rounded-2xl border-slate-200">
                <img src={c.img} alt={c.name} className="h-44 w-full object-cover" loading="lazy" />
                <CardHeader className="pb-2"><CardTitle className="text-lg">{c.name}</CardTitle><p className="text-slate-500 text-sm">{c.role}</p></CardHeader>
      ...
      <section id="results" className="py-16 bg-white/60">
        <div className="max-w-6xl mx-auto px-4">
          ...
          <p className="mt-2 text-slate-600">Our community lifts each other up — and the outcomes speak for themselves.</p>
          <div className="mt-8 grid md:grid-cols-3 gap-6">
            <Testimonial quote="Dropped 6kg and nailed my first pull‑up!" name="Jess" subtitle="Member, 8 months" />
            <Testimonial quote="Coaches actually coach — my form and confidence have skyrocketed." name="Sam" subtitle="Member, 1 year" />
            <Testimonial quote="Love the variety. I never get bored and I keep improving." name="Luca" subtitle="Member, 5 months" />
          </div>
        </div>
      </section>
        ...
        <div className="max-w-6xl mx-auto px-4 grid md:grid-cols-2 gap-8 items-start">
          <div>
            ...
            <div className="mt-4 space-y-2 text-slate-700">
              ...
              <div className="pt-2"><Button asChild variant="outline" className="rounded-2xl"><a href="https://maps.google.com/?q=66%20Edith%20St%20Wynnum%204178" target="_blank" rel="noreferrer">Get Directions</a></Button></div>
            </div>
          </div>
          <div>
